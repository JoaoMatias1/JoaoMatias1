...
font_sport = pygame.font.Font(font_path, 24)
    ...
    for sport in sports:
        ...
        if sport_div:
            ...
            for game in games:
                ...
                if len(teams) == 2:
                    team1_name = teams[0].find('span', class_='scores-text').text.strip() if teams[0].find('span',
                                                                                                           class_='scores-text') else "N/A"
                    team2_name = teams[1].find('span', class_='scores-text').text.strip() if teams[1].find('span',
                                                                                                           class_='scores-text') else "N/A"

                    team1_score_div = teams[0].find('div', class_='score-team-score')
                    team2_score_div = teams[1].find('div', class_='score-team-score')

                    team1_score = team1_score_div.find('span',
                                                       class_='scores-text').text.strip() if team1_score_div and team1_score_div.find(
                        'span', class_='scores-text') else "N/A"
                    team2_score = team2_score_div.find('span',
                                                       class_='scores-text').text.strip() if team2_score_div and team2_score_div.find(
                        'span', class_='scores-text') else "N/A"

                    team1_logo = teams[0].find('img', class_='team-logo')['src'].replace('80.80', '300.300') if teams[
                        0].find('img', class_='team-logo') else None
                    team2_logo = teams[1].find('img', class_='team-logo')['src'].replace('80.80', '300.300') if teams[
                        1].find('img', class_='team-logo') else None
    ...
    while running:
        ...
        if scores:
            ...
            if team1_logo and team2_logo:
                team1_logo = pygame.transform.scale(team1_logo, (60, 60))
                team2_logo = pygame.transform.scale(team2_logo, (60, 60))
                team1_logo.set_alpha(int(alpha))
                team2_logo.set_alpha(int(alpha))

                screen.blit(team1_logo, (CENTER[0] - 100, CENTER[1] - 50))
                screen.blit(team2_logo, (CENTER[0] + 40, CENTER[1] - 50))

            sport_text_surface = font_sport.render(sport_name, True, WHITE)
            sport_text_surface.set_alpha(int(alpha))
            sport_text_rect = sport_text_surface.get_rect(center=(CENTER[0], CENTER[1] - 120))
            ...
            team1_text_rect = team1_text_surface.get_rect(center=(CENTER[0] - 70, CENTER[1] - 90))
            ...
            team2_text_rect = team2_text_surface.get_rect(center=(CENTER[0] + 70, CENTER[1] - 90))
            ...
            score_rect = score_surface.get_rect(center=(CENTER[0], CENTER[1] + 30))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if math.hypot(event.pos[0] - BACK_BUTTON_POS[0],
                              event.pos[1] - BACK_BUTTON_POS[1]) <= BACK_BUTTON_RADIUS:
