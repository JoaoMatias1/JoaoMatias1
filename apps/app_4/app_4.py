...
font_small = load_custom_font(font_path, 20)  # Smaller font for 480x320 resolution
font_large = load_custom_font(font_path, 60)  # Adjusted larger font size
font_symbol = load_custom_font(font_path, 30)  # Smaller font for stock symbols
...
def run(screen):
    ...
    # Load background image and resize for 480x320 resolution
    ...
    while running:
        ...
        symbol_rect = symbol_surface.get_rect(center=(CENTER[0], CENTER[1] - 80))  # Adjusted Y position
...
if __name__ == "__main__":
    pygame.init()
    screen = pygame.display.set_mode((480, 320))  # Adjusted screen resolution
    pygame.display.set_caption("Stock Ticker App")
    run(screen)
    pygame.quit()
