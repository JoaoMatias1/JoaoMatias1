...
def get_icon(url):
    if url not in icon_cache:
        ...
        if response.status_code == 200:
            ...
            icon_cache[url] = pygame.transform.scale(icon_image, (48, 48))  # Reduced size for smaller resolution
        else:
...
def run(screen):
    ...
    font_large = load_custom_font(font_path_extra_bold, FONT_SIZE_LARGE)  # Load Bebas Neue font
    font_description = load_custom_font(font_path, FONT_SIZE_DESCRIPTION)
    WHITE = (255, 255, 255)

    CENTER = (screen.get_width() // 2, screen.get_height() // 2)
    INFO_RADIUS = 150  # Radius to place text elements (adjusted for smaller resolution)
    BACK_BUTTON_RADIUS = 30  # Radius for the back button
    ...
    default_background_path = os.path.join(themes_dir, "default", "background.png")
    if os.path.exists(default_background_path):
        default_background = pygame.image.load(default_background_path)
        default_background = pygame.transform.scale(default_background, (screen.get_width(), screen.get_height()))
    else:
    ...
    while running:
        ...
        if current_time >= next_update_time:
            ...
            query = "Give a very concise description of what I should wear based on this weather data: " + str(
                weather_data)
        ...
        if weather_data and 'current' in weather_data:
            ...
            _, back_button_image_path = load_theme(condition)

            if os.path.exists(back_button_image_path):
                back_button_image = pygame.image.load(back_button_image_path)
                back_button_image = pygame.transform.scale(back_button_image,
                                                           (2 * BACK_BUTTON_RADIUS, 2 * BACK_BUTTON_RADIUS))
        ...
        if 'current' in weather_data:
            ...
            temp_celsius = (weather_data['current']['temp_f'] - 32) * 5 / 9  # Convert to Celsius
            ...
            draw_text(screen, temp_text, font_large, WHITE, (CENTER[0], CENTER[1] - 30))

            # Wrap description text with adjusted bounds (limit to 3-4 lines)
            description_lines = wrap_text(description_text, font_description, screen.get_width() - 50)
            line_height = font_description.get_linesize()
            for i, line in enumerate(description_lines[:4]):  # Limit to 4 lines
                draw_text(screen, line, font_description, WHITE, (CENTER[0], CENTER[1] + 50 + i * line_height))
